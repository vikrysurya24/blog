...
@section('content')
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <form action="{{ route('roles.update', ['role' => $role]) }}" method="POST" autocomplete="off">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                                            ...
                                            <li class="list-group-item">
                                                <div class="form-check">
                                                    @if (old('permissions', $checked))
                                                        <input id="{{ $permission }}" name="permissions[]" class="form-check-input" type="checkbox" value="{{ $permission }}" {{ in_array($permission, old('permissions', $checked)) ? 'checked' : '' }}>
                                                    @else
